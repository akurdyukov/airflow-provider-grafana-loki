...
class LokiHook(HttpHook):
    ...
    :param loki_conn_id: connection that has the base API url
        i.e. https://www.grafana.com/ and optional authentication credentials.
        Default headers can also be specified in the Extra field
        in json format.
    ...
    def __init__(self, loki_conn_id: str = default_conn_name,
                 *args, **kwargs) -> None:
    ...
    def push_log(self, payload, headers=None) -> requests.Response:
        ...
        self.method = "POST"
        response = self.run(push_endpoint, data=payload,
                            extra_options={"timeout": 3}, headers=headers)
        response.raise_for_status()
        return response
